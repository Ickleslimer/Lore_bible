...
from pipeline.wiki_site_builder import slug_by_name, entity_entry_from_card
...
def test_render_quest_map_contains_character_and_year_views(tmp_path: Path) -> None:
    ...
    slug_map = slug_by_name([entity_entry_from_card({"canonical_name": "Oyuun", "entity_type": "character", "summary": ""})])
    html_doc = render_quest_map_html(catalog, slug_map=slug_map, depth=1)
...
def test_build_quest_map_site_writes_map_and_stubs(tmp_path: Path) -> None:
    seed = Path(__file__).resolve().parent / "fixtures" / "quest_song_seed.json"
    slug_map = {}
    rows = build_quest_map_site(tmp_path, slug_map=slug_map, seed_path=seed)
